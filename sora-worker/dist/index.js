...
cli
    ...
    recorder.start();
    try {
        await ensureAuthenticated(page, runtimeConfig.SORA_BASE_URL, Boolean(flags.manualLogin), artifactsDir, Boolean(flags.skipAuthCheck));
        if (flags.loginOnly) {
        ...
        if (result.metadata?.creditInfo) {
            const creditRemaining = result.metadata.creditInfo?.rate_limit_and_credit_balance?.estimated_num_videos_remaining;
