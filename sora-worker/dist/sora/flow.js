...
async function capturePageState(page, artifactsDir, label) {
    if (!artifactsDir)
        return;
...
export async function ensureAuthenticated(page, baseUrl, requireManualLogin, artifactsDir, skipAuthCheck = false) {
    ...
    try {
        await page.waitForFunction(() => !!document.querySelector('[data-testid="sora-workspace-root"]') ||
            !!document.querySelector('button[aria-label*="New video"]') ||
            !!document.querySelector('a[aria-label="Profile"]') ||
            !!document.querySelector('a[href="/profile"]'), undefined, { timeout: 120_000 });
...
export async function runGeneration(options, input) {
    ...
    await promptBox.waitFor({ state: 'visible', timeout: 30_000 }).catch(async (error) => {
        ...
        throw new Error(`Composer prompt not found: ${error.message}`);
    });
    ...
    const hasVideoPath = jobData?.encodings && typeof jobData.encodings === 'object'
        ? jobData.encodings.source?.path || jobData.url
        : false;
    if (!hasVideoPath) {
...
async function waitForJobId(page) {
    const response = await page.waitForResponse((res) => res.request().method() === 'POST' && res.url().includes('/backend/nf/create'), { timeout: 120_000 });
    try {
...
async function waitForCompletion(page, baseUrl, jobId) {
    ...
    while (Date.now() < deadline) {
        ...
        if (pollCount % 6 === 0) {
            ...
            try {
                ...
                const responsePromise = page.waitForResponse((res) => res.url().includes('/backend/project_y/profile/drafts') && res.request().method() === 'GET', { timeout: 30_000 });
                await page.goto(draftsUrl, { waitUntil: 'domcontentloaded', timeout: 60_000 });
                ...
                if (draft) {
                    ...
                    if (kind === 'sora_content_violation') {
                        const reason = (draft.markdown_reason_str ?? draft.reason_str ?? 'Content violation');
                    ...
                    if (kind === 'sora_error') {
                        const reason = (draft.markdown_reason_str ?? draft.reason_str ?? draft.error_message ?? 'Something went wrong');
                    ...
                    const encodings = draft.encodings;
                    const source = encodings && typeof encodings === 'object' ? encodings.source : undefined;
                    const videoPath = source && typeof source === 'object' ? source.path : undefined;
...
async function promoteAndCopyPublicUrl(page) {
...
async function applyComposerOptions(page, input) {
...
async function selectMenuItem(page, pattern, label) {
...
async function waitForEnabled(page, locator) {
    ...
    await page.waitForFunction((btn) => !btn.hasAttribute('disabled') && btn.getAttribute('data-disabled') !== 'true', handle, { timeout: 15_000 });
}
async function publishLatestDraft(page, baseUrl, artifactsDir) {
    const draftsUrl = new URL(DRAFTS_PATH, baseUrl).toString();
    ...
    if (await postButton.isVisible().catch(() => false)) {
        ...
        const postResponsePromise = page.waitForResponse((res) => {
            const url = res.url();
            return ((url.includes('/backend/project_y/') && res.request().method() === 'POST') ||
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        ...
        const profileResponsePromise = page.waitForResponse((res) => res.url().includes('/backend/project_y/profile') && res.request().method() === 'GET', { timeout: 15_000 });
...
async function openRelativeLink(page, locator, baseUrl) {
...
export async function checkCredits(page, baseUrl, artifactsDir) {
    try {
        // Wait for usage API response when opening usage tab (optional, don't fail if timeout)
        const usageResponsePromise = page.waitForResponse((res) => {
            const url = res.url();
            return ((url.includes('/backend/nf/') && url.includes('check')) ||
                (url.includes('/backend/project_y/') && (url.includes('usage') || url.includes('settings'))));
        }, { timeout: 10_000 }).catch(() => null); // Don't throw on timeout
        ...
        if (response) {
            ...
            const creditRemaining = payload?.rate_limit_and_credit_balance && typeof payload.rate_limit_and_credit_balance === 'object'
                ? payload.rate_limit_and_credit_balance.estimated_num_videos_remaining
                : undefined;
...
async function detectPolicyViolation(page, artifactsDir) {
