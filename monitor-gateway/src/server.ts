...
export function buildServer() {
  ...
  const latestFrames = new Map<string, StoredFrame>();
  ...
  function broadcast(frame: StoredFrame) {
  ...
  app.get('/stream/:taskId', { logLevel: 'warn' }, async (request, reply) => {
    ...
    const subs = subscribers.get(taskId) ?? new Set<Subscriber>();
    subs.add(subscriber);
  ...
  app.post('/frames', async (request, reply) => {
    ...
    const payload = request.body as Partial<FramePayload>;
