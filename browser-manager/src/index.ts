...
export async function getBrowserInstances(): Promise<BrowserInstance[]> {
  ...
  return new Promise((resolve) => {
    const processes: BrowserInstance[] = [];
    
    // Use PowerShell script file for easier debugging and maintenance
    const scriptPath = pathResolve(__dirname, '../detect-browsers.ps1');
          ...
          instances.forEach((proc: any) => {
            if (proc && proc.ProcessId) {
              ...
              if (processName.includes('Worker') || processName.includes('FastExecuteScript')) {
                ...
                // Try multiple regex patterns to match user-data-dir
                let userDataDir = '';
                const patterns = [
                  /--user-data-dir[=:]"([^"]+)"/i,
                  /--user-data-dir[=:]([^\s]+)/i,
                ...
                if (userDataDir) {
                  // Extract profile name from path like: .../profiles/profileName or .../sora2/profiles/profileName
                  ...
                  for (const pattern of profilePatterns) {
                    const match = userDataDir.match(pattern);
                    if (match && match[1]) {
                      profile = match[1];
                      break;
                    }
                  ...
                  // Fallback: if profiles found but no match, extract manually
                  if (profile === 'unknown' && userDataDir.includes('profiles')) {
                    const parts = userDataDir.replace(/\\/g, '/').split('/');
                    const profilesIdx = parts.findIndex((p: string) => p.toLowerCase() === 'profiles');
                ...
                // Final fallback: try to find profiles/xxx in command line
                if (profile === 'unknown') {
...
async function getProfilesFromMongo(): Promise<Map<string, any>> {
...
function displayInstances(instances: BrowserInstance[]) {
  console.clear();
  console.log('╔════════════════════════════════════════════════════════════╗');
...
function focusWindow(pid: number) {
