...
function mapDimensionToOrientation(dimension) {
    if (!dimension)
        return 'portrait';
    const portraitDims = ['9:16', '3:4'];
    const landscapeDims = ['16:9', '21:9', '4:3'];
    if (portraitDims.includes(dimension))
        return 'portrait';
    if (landscapeDims.includes(dimension))
        return 'landscape';
    // Default to portrait if unknown
    return 'portrait';
}
// Map timing to duration (must be 10 or 15)
function mapTimingToDuration(timing) {
    if (!timing)
        return 15;
...
async function processTask() {
    ...
    try {
        ...
        logger.info({ taskId: task.id, profile: profile.name, duration, orientation }, 'Starting worker');
        // Run worker
